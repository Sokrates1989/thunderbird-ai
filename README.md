...
Release 3.3.4 saves both dashboard and single-message launch modes immediately
when their selectors change. The choices therefore survive installer updates
without depending on the separate Save button, while a failed write visibly
restores the last persisted value. Installer tests now also protect existing
Thunderbird profile storage. The settings cleanup from 3.3.3 remains included.
...
- Global unread-inbox dashboard with complete header pagination, account or
  combined latest-50 views, sender/date/score filters, sorting, bulk actions,
  and configurable local content previews.
...
1. Download and run `Thunderbird-AI-Setup-3.3.4-win-x64.exe`.
...
2. Open `Thunderbird-AI-Setup-3.3.4-macos.pkg`.
