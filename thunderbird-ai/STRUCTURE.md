...
- `LaunchModeService.js` owns normalized overlay/tab preferences and transient popup routing. Startup-critical shared services attach directly to `globalThis` so Thunderbird's generated background page cannot expose a cross-script lexical binding in its temporal dead zone. Neither toolbar declares a static manifest popup: synchronous background listeners wake the event page, read the relevant persisted mode, assign an overlay only for that click, and clear it immediately afterward. Dashboard and single-message mode are stored independently.
