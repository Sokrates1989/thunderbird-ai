...
- `GlobalMailService.js` owns Thunderbird header pagination, previews, deletion, and fault-isolated read-state updates through the `messagesUpdate` permission.
...
- `ScoreFeedbackEditor.js` and `score-feedback-editor.css` provide the shared importance/spam correction fields, reason categories, and separate free-text explanations used by the dashboard, single-message scoring, and Settings archive.
