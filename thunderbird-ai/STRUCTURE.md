...
- `DashboardLaunchService.js` owns the saved overlay/tab launch mode, dynamically enables or disables the global toolbar popup, focuses an existing durable dashboard tab before creating a fallback, and coalesces concurrent launches. Every Thunderbird tab/window API call is bounded so a pending promise cannot block future toolbar clicks until restart. The service persists only the latest content-free launch diagnostic, which `DashboardLaunchSettingsComponent.js` exposes for refresh and copy beside the launch preference. The single-message footer reuses this boundary. `DashboardLaunchPromptComponent.js` maintains the local three-use/five-open guidance counters and presents optional prompts without interrupting an already open dashboard dialog.
