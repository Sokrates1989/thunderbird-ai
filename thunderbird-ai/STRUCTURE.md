...
- `DashboardLaunchService.js` owns the saved overlay/tab launch mode, dynamically enables or disables the global toolbar popup, focuses an existing durable dashboard tab before creating a fallback, coalesces concurrent launches, and maintains the local three-use/five-open guidance counters. The single-message footer reuses this boundary. `DashboardLaunchPromptComponent.js` presents optional prompts without interrupting an already open dashboard dialog, while `DashboardLaunchSettingsComponent.js` exposes the same preference in Settings.
