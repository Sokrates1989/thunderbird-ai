...
`Thunderbird-AI-Setup-3.2.0-macos.pkg`. It uses only the current-user macOS
...
1. Start Thunderbird at least once, then open an email and an unsaved test
   draft.
2. Open `artifacts/Thunderbird-AI-Setup-3.2.0-macos.pkg`, read and accept the GPL,
...
5. Select OpenAI, Claude, Mistral, DeepSeek, and the custom endpoint in settings;
   ...
   and run the API test. The single-message popup must show **Version 3.2.0**.
