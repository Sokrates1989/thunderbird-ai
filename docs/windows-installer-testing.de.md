...
`Thunderbird-AI-Setup-3.8.2-win-x64.exe`. Es installiert das Add-on nur für den
...
6. In den Einstellungen OpenAI, Claude, Mistral, DeepSeek und den individuellen
   ...
   API-Test aufrufen. Im Einzelmail-Popup muss **Version 3.8.2** stehen und die
...
10. Den Einzelmail-Assistenten in einem eigenen Tab öffnen und im Kopf der
    erweiterten Ansicht **Zum kompakten Overlay** ausführen. Thunderbird muss
    den Ausgangs-Nachrichten-Tab wiederherstellen, dort das Overlay öffnen und
    den erweiterten Tab schließen. Wurde der Ausgangs-Tab geschlossen, muss der
    Arbeitsbereich zum nächstgelegenen vorherigen Tab schließen, ohne das
    Overlay für eine andere Nachricht zu öffnen.
