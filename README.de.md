...
> **[The complete README is also available in English.](README.md)**

Ein Thunderbird-MailExtension-Add-on für Zusammenfassungen, Antwortentwürfe und weitere E-Mail-Analysen mit wählbaren AI-Anbietern.
...
anklickbare Anleitung für OpenAI, Claude (Anthropic), Mistral, DeepSeek und
individuelle Endpunkte. Die
...
## Umfang von Release 3.10.0

Release 3.10.0 hält die Aktionen für ausgewählte Dashboard-Nachrichten
erreichbar, wenn beide vollständigen Sammelaktionsleisten außerhalb des
sichtbaren Bereichs liegen. Ein kompakter schwebender Stapel zeigt die aktuelle
Auswahlanzahl und dieselben fünf Aktionen zum Analysieren, Neubewerten,
Gelesen-Markieren, Archivieren und Löschen mit übereinstimmenden Symbolen,
Farben, Deaktivierungszuständen und lokalisierten Tooltips bei Hover oder
Tastaturfokus. Die schwebenden Aktionen bleiben ohne Auswahl oder bei sichtbarer
vollständiger Aktionsleiste ausgeblendet.

## Umfang von Release 3.9.0

Release 3.9.0 ergänzt eine dritte Startoption für einzelne Nachrichten: ein
dauerhaft geöffnetes kompaktes Fenster, das beim Arbeiten in Thunderbird offen
bleibt und nur über seine Schließen-Schaltfläche oder die nativen Fenster-
Steuerelemente geschlossen wird. Das ursprüngliche schlanke Overlay bleibt die
Vorgabe; der eigene Thunderbird-Tab bleibt ebenfalls verfügbar. Kompakte
Schaltflächen in jeder Ansicht öffnen die jeweils anderen beiden Modi, ohne die
gespeicherte Vorgabe zu ändern. Das letzte AI-Ergebnis und abgeschlossene Chat-
Beiträge bleiben pro Nachricht für die aktuelle Thunderbird-Sitzung erhalten,
sodass Schließen, erneutes Öffnen und Ansichtswechsel sie nicht verwerfen.
...
Konfigurationsseite griffbereit. Übereinstimmende Aktionsleisten stehen jetzt
über und unter dem Formular; bei ungespeicherten Änderungen erscheint zusätzlich
die überarbeitete schwebende Aktion **Einstellungen speichern**. Ein erfolgreicher
API-Verbindungstest speichert die exakt getestete Anbieterkonfiguration
automatisch. Die schwebende Aktion bestätigt den Erfolg kurz, während das
Formular nach einem fehlgeschlagenen Test oder Speichervorgang sichtbar
ungespeichert bleibt. Die Nach-oben-Aktionen in Dashboard und Einzelmail-Ansicht
verwenden dieselbe verfeinerte Darstellung.

## Umfang von Release 3.7.0

Release 3.7.0 merkt sich die zuletzt über `+` oder `−` gewählte Höhe einer
Dashboard-Vorschau. Beide Steuerelemente ändern die ausgewählte Vorschau in
Vier-Zeilen-Schritten innerhalb des Bereichs von 1–20 Zeilen und speichern die
Höhe als Vorgabe für die nächste Vorschau – auch über einen Thunderbird-Neustart
hinweg. Bereits geöffnete Vorschauen behalten ihre unabhängige Höhe.
...
Auswahl **Gelesene Nachrichten einbeziehen**. Standard bleibt ausschließlich
ungelesen. Nach dem Aktivieren fragt das Add-on gelesene und ungelesene
Posteingangs-Kopfzeilen neu ab, behält die Auswahl für die aktuelle
Thunderbird-Sitzung und nennt den erweiterten Bestand in der Filterübersicht.
Gelesene Zeilen verwenden normale Betreffschrift und bieten **Als ungelesen
markieren** an.

## Umfang von Release 3.5.7

Release 3.5.7 stellt die Auswahlfläche über die volle Höhe jeder
Dashboard-Nachricht wieder her. Ein Klick auf freie Bereiche der Nachrichtenkachel
wählt die Nachricht wieder aus oder ab, während Aktionsschaltflächen und die
optionalen Steuerelemente der Nachrichtenvorschau unabhängig bleiben. Dieses
Patch-Release wird über Thunderbird Add-ons verteilt und erzeugt keinen neuen
nativen Installer.

## Umfang von Release 3.5.6

Release 3.5.6 macht die Lesestatus-Aktion im Einzelmail-Arbeitsbereich umkehrbar.
Eine ungelesene Nachricht zeigt die grüne Aktion **Als gelesen markieren**; eine
gelesene Nachricht zeigt stattdessen die bernsteinfarbene Aktion **Als ungelesen
markieren**. Nach beiden Aktionen werden Thunderbirds tatsächlicher
Nachrichtenstatus, Detailanzeige, Beschriftung, Symbol, zugängliche Beschreibung
und Farbe gemeinsam aktualisiert, ohne den Arbeitsbereich neu zu öffnen.

Die eingeklappte Dashboard-Ansicht bleibt ebenfalls auf einen Blick
verständlich. Die Ergebniszeile nennt nun die getrennte oder kombinierte
Kontenansicht und hebt die aktive Sortierung hervor. Die kontextbezogene
Filteranzeige nennt nur aktive Absender-, Datums-, AI-Status- und Mindestwertfilter
zusammen mit der Zurücksetzen-Aktion. Diese Eingrenzungen gelten weiterhin nur
für die aktuelle Thunderbird-Sitzung. Die globale Anzeige und die Kontenüberschrift
vergleichen nun einheitlich die sichtbaren Zeilen mit dem vollständigen ungelesenen
Ausgangsbestand, zum Beispiel `Angezeigt: 4 von 15`.
...
- standardmäßig Luna-basierte Bulk-Auswertung noch nicht bewerteter ausgewählter Dashboard-Nachrichten mit Wichtigkeit, Spam-Wahrscheinlichkeit und Risikowert von 0–100 %, sitzungsgebundenen Score-Filtern, Score-Sortierungen und ausdrücklicher Neu-Bewertung
...
- jede sichtbare Einzelvorschau lässt sich unabhängig über `+` und `−` in Vier-Zeilen-Schritten innerhalb von 1–20 Zeilen anpassen; die zuletzt gewählte Höhe wird dauerhaft als Vorgabe für die nächste Vorschau gespeichert, während Schließen und Öffnen der Originalmail weiter verfügbar bleiben
...
OpenAI bleibt der Standardanbieter und nutzt wie bisher die Responses API mit
`store: false`. Claude verwendet die Anthropic Messages API, Mistral und
DeepSeek verwenden ihre OpenAI-kompatiblen Chat-Completions-APIs. Für Claude
ist ein separater Anthropic API-Schlüssel erforderlich; eine Anmeldung oder ein
...
Vorgabe des aktuellen Anbieters. Bei OpenAI entsprechen diese Rollen:
...
Kategorisierung, bisherige Wichtigkeits- und Spam-Analyse, Übersetzung, Extraktion und Textverbesserung besitzen ebenfalls eine eigene Modellauswahl. Jede vom gewählten Anbieter unterstützte Modell-ID kann manuell eingetragen werden. Bulk-Aufrufe verarbeiten höchstens acht Nachrichten pro API-Aufruf bei maximal zwei parallelen Aufrufen.
...
Temporäre Netzwerkfehler, Zeitüberschreitungen, Anfragelimits und vorübergehende Anbieterfehler werden mit begrenzten Wartezeiten automatisch bis zu zweimal wiederholt. Ein `Retry-After`-Hinweis wird bis zu zehn Sekunden berücksichtigt. Fehlerhafte oder unberechtigte API-Schlüssel, ausgeschöpftes Guthaben und dauerhafte Clientfehler werden ohne nutzlose Wiederholung eindeutig gemeldet. Thunderbird-Verbindungsfehler werden nur dann wiederholt, wenn Thunderbird bestätigt, dass die Nachricht den Hintergrundprozess nicht erreicht hat.
...
5. Mit den Kalenderfeldern **Datum von** und **Datum bis** einen inklusiven Bereich einstellen. Außerhalb liegende Nachrichten müssen verschwinden; ein Bis-Datum vor dem Von-Datum muss abgewiesen werden. Danach **Gelesene Nachrichten einbeziehen** aktivieren: Gelesene und ungelesene Posteingangs-Mails müssen neu geladen werden, gelesene Betreffe normale Schrift verwenden und **Als ungelesen markieren** anbieten. Nach dem Zurücksetzen muss wieder nur der ungelesene Standardbestand erscheinen.
...
39. Unter **Einstellungen → AI Assistant öffnen** die Modi für **Globale Posteingangsübersicht** und **Assistant für eine geöffnete E-Mail** unabhängig auswählen. Für das Dashboard zunächst **Als kompaktes Overlay öffnen** speichern und fünfmal das globale Toolbar-Symbol verwenden. Beim fünften tatsächlichen Öffnen muss ein lokalisierter Hinweis mit **Später**, **Jetzt ausprobieren** und **Diesen Hinweis nicht mehr anzeigen** erscheinen. **Später** muss einen neuen Fünfer-Zyklus beginnen; **Jetzt ausprobieren** muss den dauerhaften Tab öffnen, ohne den Standard zu ändern. Anschließend dreimal das Vollbildsymbol im Overlay verwenden. Beim dritten Mal muss der geöffnete Tab anbieten, künftig immer direkt im Tab zu starten. **Immer im Tab öffnen** auswählen und prüfen, dass weitere Toolbar-Klicks ohne vorgeschaltetes Overlay direkt den vorhandenen Thunderbird-Tab fokussieren. Danach das Dashboard wieder auf Overlay stellen und für den Einzelmail-Assistenten nacheinander **kompaktes Overlay**, **dauerhaft geöffnetes kompaktes Fenster** und **eigener Thunderbird-Tab** als Vorgabe prüfen. In jeder Einzelmail-Ansicht müssen oben rechts genau die beiden anderen Ziele angeboten werden. Beim Wechsel durch alle drei Ansichten müssen Quellnachricht, fertiges Ergebnis und Chatverlauf erhalten bleiben; das normale Overlay darf beim Fokuswechsel schließen, während das dauerhafte Fenster bis zur **×**-Schaltfläche oder zum nativen Schließen geöffnet bleibt. Ein temporärer Wechsel darf die gespeicherte Vorgabe nicht verändern. Bei aktivierter Option **Diesen Hinweis nicht mehr anzeigen** darf der jeweilige Dashboard-Hinweis auch nach weiteren Öffnungen nicht zurückkehren.
...
44. Mindestens eine Dashboard-Nachricht auswählen und so scrollen, dass weder die obere noch die untere Sammelaktionsleiste sichtbar ist. Rechts muss ein schwebender Stapel mit Auswahlanzahl sowie denselben fünf Symbolen, Farben und Aktionen für **Unbewertete Auswahl analysieren**, **Auswahl neu bewerten**, **Ausgewählte als gelesen markieren**, **Ausgewählte archivieren** und **Ausgewählte löschen** erscheinen. Hover und Tastaturfokus müssen den lokalisierten Aktionsnamen zeigen. Sobald eine vollständige Sammelaktionsleiste sichtbar wird oder die Auswahl leer ist, muss der Stapel verschwinden; während einer laufenden Aktion müssen seine Schaltflächen deaktiviert sein.

Im Einzelmail-Popup wird die aktive Add-on-Version unter dem Betreff angezeigt. Nach einem Update muss dort **Version 3.10.0** stehen. Das Dashboard verwendet standardmäßig den Ungelesen-Status als Kandidatenfilter; **Gelesene Nachrichten einbeziehen** erweitert den Posteingangsbestand nur für die aktuelle Thunderbird-Sitzung. Für die im Dashboard ausgewerteten Nachrichten bleiben die AI-Scores lokal gespeichert und erlauben den Filter **Nur nicht analysierte**; Nachrichten, die außerhalb des Dashboards analysiert wurden, erhalten dadurch jedoch keine Dashboard-Markierung.
...
- `common/`: Hintergrundskript sowie Storage-, Nachrichten- und anbieteroffene AI-Dienste
...
AI Mail Assistant for Thunderbird ist freie Open-Source-Software unter der [GNU General Public License Version 3 oder neuer](LICENSE). Forks und Änderungen sind ausdrücklich erlaubt; Beiträge als [Pull Request](CONTRIBUTING.de.md) sind besonders willkommen. Für Sicherheitsmeldungen gilt die [Security Policy](SECURITY.de.md). Die für eine Veröffentlichung relevante Datenverarbeitung beschreibt die [Privacy Policy](PRIVACY.de.md); Store-Metadaten und Reviewer-Schritte enthält das [Thunderbird-Add-ons-Einreichungsblatt](docs/atn-submission.de.md).
