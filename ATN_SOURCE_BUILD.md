...
This source archive corresponds to AI Mail Assistant for Thunderbird 3.8.1.
...
```bash
mkdir -p artifacts
./build-addon.sh --output artifacts/thunderbird-ai-3.8.1.xpi
```
...
license, and generates `install-defaults.json` for version 3.8.1.

## Validate

```bash
npm test
unzip -t artifacts/thunderbird-ai-3.8.1.xpi
unzip -p artifacts/thunderbird-ai-3.8.1.xpi manifest.json
```

The packaged manifest must report version `3.8.1`, extension ID
