...
This source archive corresponds to AI Mail Assistant for Thunderbird 3.4.2.
...
```bash
mkdir -p artifacts
./build-addon.sh --output artifacts/thunderbird-ai-3.4.2.xpi
```
...
license, and generates `install-defaults.json` for version 3.4.2.

## Validate

```bash
npm test
unzip -t artifacts/thunderbird-ai-3.4.2.xpi
unzip -p artifacts/thunderbird-ai-3.4.2.xpi manifest.json
```

The packaged manifest must report version `3.4.2`, extension ID
