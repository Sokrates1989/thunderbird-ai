...
This source archive corresponds to AI Mail Assistant for Thunderbird 3.10.0.
...
- Linux, macOS, or Windows 11 through WSL 2;
- Bash;
- Node.js 20 or newer;
- `zip` and `unzip`.

No package-manager installation or network access is required to build the XPI.
On Windows, open WSL, change to the repository through its `/mnt/<drive>/...`
path, and run the same commands below. The repository enforces LF line endings
for shell scripts so the staged reviewer build is reproducible from Windows.
...
```bash
mkdir -p artifacts
./build-addon.sh --output artifacts/thunderbird-ai-3.10.0.xpi
```
...
license, and generates `install-defaults.json` for version 3.10.0.

## Validate

```bash
npm test
unzip -t artifacts/thunderbird-ai-3.10.0.xpi
unzip -p artifacts/thunderbird-ai-3.10.0.xpi manifest.json
```

The packaged manifest must report version `3.10.0`, extension ID
